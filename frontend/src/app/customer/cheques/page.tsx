...
export default function CustomerChequesPage() {
  const accountHref = "/customer/account?tab=bank-accounts";
  const deliveryAddressHref = "/customer/account?tab=delivery-addresses";
          ...
          >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
              ...
              <div className="p-5 bg-gradient-to-br from-[#0A3D8F]/5 to-slate-50 rounded-xl border border-[#0A3D8F]/10">
                ...
                <div className="mt-4 pt-4 border-t border-[#0A3D8F]/10">
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 mb-2">Cheque Type</p>
                  {(() => {
                    const full = selectedChequeFull as ApiCheque | null;
                    ...
                    return (
                      <>
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          {chequeType === "original" ? (
                            ...
                            <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full bg-red-600 text-white">
                              <i className="ri-arrow-go-back-line"></i> Returned Cheque
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-0.5 rounded-full bg-slate-200 text-slate-600">
                              <i className="ri-question-line"></i> Not Classified
                            </span>
                          )}
                        ...
                        {typeClassification?.reasoning && (
                          <p className="text-xs text-slate-600 leading-relaxed">{typeClassification.reasoning}</p>
                        )}
                        {chequeType === "returned" && Array.isArray(typeClassification?.indicators) && typeClassification.indicators.length > 0 && (
                          <ul className="mt-2 space-y-1">
                            {typeClassification.indicators.map((ind: string, i: number) => (
