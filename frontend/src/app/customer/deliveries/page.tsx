...
function statusMeta(status: DeliveryDto["status"]): {
  ...
  switch (status) {
    case "pending":    return { label, pillClass: "bg-amber-50 text-amber-700 ring-1 ring-amber-200",   accentClass: "bg-amber-500",  borderClass: "border-l-amber-400" };
    case "approved":   return { label, pillClass: "bg-blue-50 text-blue-700 ring-1 ring-blue-200",     accentClass: "bg-blue-600",   borderClass: "border-l-blue-500" };
    case "in_transit": return { label, pillClass: "bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200", accentClass: "bg-indigo-600", borderClass: "border-l-indigo-500" };
    case "delivered":  return { label, pillClass: "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200", accentClass: "bg-emerald-600", borderClass: "border-l-emerald-500" };
    case "rejected":   return { label, pillClass: "bg-rose-50 text-rose-700 ring-1 ring-rose-200",     accentClass: "bg-rose-600",   borderClass: "border-l-rose-500" };
    case "cancelled":  return { label, pillClass: "bg-slate-100 text-slate-700 ring-1 ring-slate-200", accentClass: "bg-slate-500",  borderClass: "border-l-slate-300" };
    default:           return { label, pillClass: "bg-slate-100 text-slate-700 ring-1 ring-slate-200", accentClass: "bg-slate-400",  borderClass: "border-l-slate-200" };
  }
}
...
function CustomerDeliveryRequestsContent() {
  ...
  // Auto-scroll to highlighted row after data loads
  useEffect(() => {
  ...
  const handleConfirmReceipt = async (id: string) => {
    setConfirmingId(id);
    try {
      await deliveriesApi.confirmReceipt(id);
      setRows((prev) =>
        prev.map((r) => r.id === id ? { ...r, customerConfirmedAt: new Date().toISOString() } : r)
      );
    } catch (err: any) {
          ...
          <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
            ...
            <div className="divide-y divide-slate-100">
              {filtered.map((r) => {
                ...
                const confirmed = Boolean(r.customerConfirmedAt);
                const addressLine = [
                  r.addressLine1,
                  r.addressLine2,
                  r.addressCity,
                ...
                return (
                  <div
                    ...
                    className={`group flex items-stretch border-l-[3px] ${s.borderClass} hover:bg-slate-50/80 transition-colors ${
                      highlighted ? "bg-[#0A3D8F]/5 ring-1 ring-inset ring-[#0A3D8F]/20" : ""
                    }`}
                  >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          ...
                          {r.status === "pending" && (
                            <button
                              type="button"
                              onClick={(e) => { e.stopPropagation(); void onCancel(r); }}
                          ...
                          {/* Confirm Receipt button — shown for delivered deliveries */}
                          {r.status === "delivered" && !confirmed && (
                            <button
                              type="button"
                              disabled={confirmingId === r.id}
                              onClick={(e) => { e.stopPropagation(); void handleConfirmReceipt(r.id); }}
                              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-emerald-600 text-white text-[12px] font-bold hover:bg-emerald-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                            >
                              <i className="ri-checkbox-circle-line text-sm" />
                              {confirmingId === r.id ? "Confirming…" : "Got the Package"}
                            </button>
                          )}

                          {/* Already confirmed badge */}
                          {r.status === "delivered" && confirmed && (
                            <span className="inline-flex items-center gap-1 text-[11px] text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200 font-semibold">
